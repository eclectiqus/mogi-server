...
function resizeImageToSize(path, width, height, outputTempFilePath, outputFilePath) {
  im.resize({
    srcPath: path,
    width: width,
    height: height,
...
resizeImg = function(basePath, image){
    ...
    mkdirp(smallPath, function (err) {
        if (err) {
          ...
          resizeImageToSize(basePath+"/original/"+image, 40, 40, smallOutputTempFilePath, smallOutputFilePath);
        }
    });
    mkdirp(mediumPath, function (err) {
        if (err) {
          ...
          resizeImageToSize(basePath+"/original/"+image, 100, 100, mediumOutputTempFilePath, mediumOutputFilePath);
        }
    });
};
