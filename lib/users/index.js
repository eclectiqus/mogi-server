...
resizeImg = function(basePath, image){
    ...
    var iconPath = path.join(basePath,'icon');
    ...
    var iconOutputFilePath = path.join(iconPath,image);
    var iconOutputTempFilePath = path.join(iconPath,"temp"+image);
    ...
    mkdirp(iconPath, function (err) {
      if (err) {
        console.error('resizeImg err='+err+']');
        return;
      } else {
        resizeImageToSize(basePath+"/original/"+image, 80, iconOutputTempFilePath, iconOutputFilePath);
      }
    });
};
